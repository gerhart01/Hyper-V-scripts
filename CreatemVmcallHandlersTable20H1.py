...
hv_stats = GetEnum("HV_HYPERCALL_STATS_GROUP")  # hypercall's category using for statistic purpose
...
def add_hypercall_stats_enum(hv_call_count):
    ...
    if hv_stats != 0xffffffffffffffffL:
        print "enum HV_HYPERCALL_STATS_GROUP already exists"

    if hv_stats == 0xffffffffffffffffL:
        hv_stats = AddEnum(-1, "HV_HYPERCALL_STATS_GROUP", 0)
        BeginTypeUpdating(UTP_ENUM)
        AddConst(hv_stats, "GPA_SPACE_HYPERCALL", 0x3D + win_20h1_offset)
        AddConst(hv_stats, "LOGICAL_PROCESSOR_HYPERCALL", 0x3E + win_20h1_offset)
        AddConst(hv_stats, "LONG_SPIN_WAIT_HYPERCALL", 0x3F + win_20h1_offset)
        AddConst(hv_stats, "OTHER_HYPERCALL", 0x40 + win_20h1_offset)
        AddConst(hv_stats, "INTER_PARTITION_COMMUNICATION_HYPERCALL",
                 0x41 + win_20h1_offset)  # i don't see same counter in PerformanceMonitor, but there is category in Hyper-v TLFS.
        AddConst(hv_stats, "VIRTUAL_INTERRUPT_HYPERCALL", 0x42 + win_20h1_offset)
        AddConst(hv_stats, "VIRTUAL_MMU_HYPERCALL", 0x43 + win_20h1_offset)
        AddConst(hv_stats, "VIRTUAL_PROCESSOR_HYPERCALL", 0x44 + win_20h1_offset)
        AddConst(hv_stats, "VIRTUAL_PROCESSOR_HYPERCALL02", 0x45 + win_20h1_offset)
        AddConst(hv_stats, "FLUSH_PHYSICAL_ADDRESS_SPACE", 0x8F + win_20h1_offset)
        AddConst(hv_stats, "FLUSH_PHYSICAL_ADDRESS_LIST", 0x90 + win_20h1_offset)
        EndTypeUpdating(UTP_ENUM)

def get_hypercall_count(table_address):
    count = 1
    while True:
        hv_call_number = idc.Dword(table_address+8+count*TableEntrySize)
        if hv_call_number == 0:
...
OpEnumEx(StartAddress+8+j*2, 0, hv_stats, 0)
...
for i in range(1,hvCallCount+1):
    ...
    OpEnumEx(StartAddress+i*TableEntrySize+8+j*2, 0, hv_stats, 0)