...
def get_vmcs_code(text,vcode):
	SetRegValue(vcode,'rax')
	StepInto()
	GetDebuggerEvent(STEP,-1)
	print text, ' = 0x%x' % GetRegValue('rax')
	StepInto()
	GetDebuggerEvent(STEP,-1)
...
def get_vmcs_table():
    ...
    SetRegValue(vmcs_show,'rip')
    keys = vmcs_dict.keys()
    keys.sort()
    for key in keys:
        get_vmcs_code(key,vmcs_dict[key])
    tmp = vmcs_show
    for code_byte in backup_opcode:
		PatchByte(tmp, code_byte)
		tmp = tmp + 1
    SetRegValue(RipBackup,'rip')
    SetRegValue(RaxBackup,'rax')
    #Eval('SendDbgCommand("r efl=0x'+str(EflBackup)+'")')
    StepInto()
    GetDebuggerEvent(STEP,-1)
